...
- 🟡 Tower Defense deep factory: visual enemy/tower catalogs, authored waves, pure first/last/strongest/weakest/nearest targeting, visual placement/bounds rules, range/damage combat resolution, pure enemy path progression, timed status effects, projectile timing/impact resolution, portable upgrade progression, deterministic procedural-wave formulas, and a deterministic wave simulator are generated now. Visual path/upgrade/effect authoring, replication, procedural-wave configuration, and the 100-wave dashboard remain.
