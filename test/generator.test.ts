...
describe("Tower Defense generator", () => {
  it("emits a normal Rojo/Wally/Luau project", () => {
    ...
    expect(files.find((file) => file.path === "src/shared/domain/TowerDefenseConfig.luau")?.content).toContain("proceduralWaves = { enabled = false");
  ...
  it("generates authored tower, enemy, and wave data", () => {
    ...
    expect(files.find((file) => file.path === "src/shared/domain/Upgrades.luau")?.content).toContain("function Upgrades.nextStep");
    expect(files.find((file) => file.path === "src/shared/domain/ProceduralWaves.luau")?.content).toContain("function ProceduralWaves.create");
