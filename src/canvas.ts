...
export class ForgeCanvasProvider implements vscode.WebviewViewProvider {
  ...
  private render() {
    ...
    const projectName = definition?.name ?? "No Forge definition yet";
    const targetModes = definition?.targeting.join(" · ") ?? "Create a definition to begin";
    const networkNode = definition?.messages.map((message) => message.name).join(" · ") || "No schemas";
...
<html><head><meta charset="utf-8"><style>
  ...
  .toolbar { display:flex; gap:6px; padding:10px 16px; } button { color:var(--vscode-button-foreground); background:var(--vscode-button-background); border:0; border-radius:4px; padding:6px 8px; cursor:pointer; font-size:11px; } button:hover{background:var(--vscode-button-hoverBackground)}
  ...
  <main class="canvas">
    ...
    <section class="node factory"><h2>Factory · Tower Defense</h2><p>${definition.topology} topology<br>${definition.economy} economy</p><span class="tag">${targetModes}</span></section>
  ...
  </main><footer class="footer">Visual definitions are source. Generated Luau remains version-controllable and editable.</footer>
  <script>const vscode = acquireVsCodeApi(); document.querySelectorAll('button').forEach((button) => button.addEventListener('click', () => vscode.postMessage({command:button.dataset.command})));</script>
</body></html>`;
  }
}
