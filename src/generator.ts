...
const systemFunctions: Record<TowerDefenseDefinition["flow"][number]["kind"], { label: string; wave: string }> = {
  spawnWave: { label: "Spawn Wave", wave: "state.wave + 1" },
  moveEnemies: { label: "Move Enemies", wave: "state.wave" },
  acquireTargets: { label: "Acquire Targets", wave: "state.wave" },
  attackTargets: { label: "Attack Targets", wave: "state.wave" },
  applyDamage: { label: "Apply Damage", wave: "state.wave" },
  cleanupDead: { label: "Cleanup Dead", wave: "state.wave" }
};
...
export function generateTowerDefense(definition: TowerDefenseDefinition): GeneratedFile[] {
  ...
  const enabledBlocks = definition.flow.filter((block) => block.enabled);
  const helpers = enabledBlocks.map((block) => {
    const system = systemFunctions[block.kind];
    return `local function ${functionName(block.id)}(state: State): State\n\t-- ${system.label} block (${block.id})\n\treturn { wave = ${system.wave}, currency = state.currency, lives = state.lives, tick = state.tick + 1, lastSystem = ${quote(system.label)} }\nend`;
  }).join("\n\n");
