...
${packagesFor(realm)}`).join("\n\n");
  ...
  const scenarioTests = definition.scenarios.map((scenario) => `test.case(${quote(scenario.name)}, function(asserts)\n\tlocal summary = TowerDefenseSimulator.simulate(${scenario.runs})\n\tasserts.truthy(summary.${scenario.metric} ${scenario.comparison} ${scenario.amount})\nend)`).join("\n\n");
  ...
  return [
    ...
    { path: "src/shared/domain/TowerDefenseConfig.luau", content: `-- Generated by Roblox Forge. Use the named user-code region for supported custom logic.\n\nexport type TargetMode = ${definition.targeting.map(quote).join(" | ")}\n\n-- <forge:user-code id="targeting-extensions">\n-- Add pure targeting helpers here. This region survives regeneration.\n-- </forge:user-code>\n\nreturn {\n\ttopology = ${quote(definition.topology)},\n\teconomy = ${quote(definition.economy)},\n\tplacement = { width = ${definition.placement.width}, depth = ${definition.placement.depth}, minimumSeparation = ${definition.placement.minimumSeparation} },\n\ttargetModes = { ${targetModes} },\n}\n` },
    { path: "src/shared/domain/Targeting.luau", content: `-- Generated pure targeting behavior. Candidates use portable numeric fields, not Roblox Instances.\n\nexport type Candidate = { id: string, progress: number, health: number, distance: number }\nexport type TargetMode = "first" | "last" | "strongest" | "weakest" | "nearest"\n\nlocal Targeting = {}\n\nfunction Targeting.select(candidates: { Candidate }, mode: TargetMode): Candidate?\n\tlocal selected: Candidate? = nil\n\tfor _, candidate in ipairs(candidates) do\n\t\tif selected == nil then\n\t\t\tselected = candidate\n\t\telseif mode == "first" and candidate.progress > selected.progress then\n\t\t\tselected = candidate\n\t\telseif mode == "last" and candidate.progress < selected.progress then\n\t\t\tselected = candidate\n\t\telseif mode == "strongest" and candidate.health > selected.health then\n\t\t\tselected = candidate\n\t\telseif mode == "weakest" and candidate.health < selected.health then\n\t\t\tselected = candidate\n\t\telseif mode == "nearest" and candidate.distance < selected.distance then\n\t\t\tselected = candidate\n\t\tend\n\tend\n\treturn selected\nend\n\nreturn Targeting\n` },
    { path: "src/shared/domain/Placement.luau", content: `-- Generated pure placement validation. Coordinates are portable numbers, not Roblox Instances.\n\nexport type Point = { x: number, z: number }\n\nlocal Placement = {}\n\nfunction Placement.isAvailable(candidate: Point, occupied: { Point }, minimumSeparation: number): boolean\n\tassert(minimumSeparation >= 0, "minimumSeparation cannot be negative")\n\tfor _, point in ipairs(occupied) do\n\t\tlocal dx = candidate.x - point.x\n\t\tlocal dz = candidate.z - point.z\n\t\tif dx * dx + dz * dz < minimumSeparation * minimumSeparation then\n\t\t\treturn false\n\t\tend\n\tend\n\treturn true\nend\n\nfunction Placement.isInsideBounds(candidate: Point, width: number, depth: number): boolean\n\treturn candidate.x >= 0 and candidate.z >= 0 and candidate.x <= width and candidate.z <= depth\nend\n\nreturn Placement\n` },
    { path: "src/shared/domain/Combat.luau", content: `-- Generated pure combat resolution. Rendering projectiles is a Roblox adapter concern.\n\nexport type TowerAttack = { damage: number, range: number }\nexport type Target = { health: number, distance: number }\nexport type HitResult = { health: number, defeated: boolean }\n\nlocal Combat = {}\n\nfunction Combat.canAttack(tower: TowerAttack, target: Target): boolean\n\treturn tower.damage > 0 and target.health > 0 and target.distance <= tower.range\nend\n\nfunction Combat.applyDamage(target: Target, damage: number): HitResult\n\tassert(damage >= 0, "damage cannot be negative")\n\tlocal health = math.max(0, target.health - damage)\n\treturn { health = health, defeated = health == 0 }\nend\n\nreturn Combat\n` },
    { path: "src/shared/domain/PathProgress.luau", content: `-- Generated pure enemy path progression. Roblox path visuals belong in an adapter.\n\nexport type Progress = { value: number, reachedEnd: boolean }\n\nlocal PathProgress = {}\n\nfunction PathProgress.advance(value: number, speed: number, deltaTime: number, pathLength: number): Progress\n\tassert(speed >= 0 and deltaTime >= 0 and pathLength > 0, "invalid path progression inputs")\n\tlocal nextValue = math.min(pathLength, value + speed * deltaTime)\n\treturn { value = nextValue, reachedEnd = nextValue >= pathLength }\nend\n\nfunction PathProgress.normalized(value: number, pathLength: number): number\n\tassert(pathLength > 0, "pathLength must be positive")\n\treturn math.clamp(value / pathLength, 0, 1)\nend\n\nreturn PathProgress\n` },
    { path: "src/shared/domain/StatusEffects.luau", content: `-- Generated pure timed status-effect mechanics. Visuals and particles belong in a Roblox adapter.\n\nexport type Effect = { id: string, remaining: number, damagePerSecond: number, speedMultiplier: number }\nexport type TickResult = { health: number, effects: { Effect }, speedMultiplier: number }\n\nlocal StatusEffects = {}\n\nfunction StatusEffects.upsert(effects: { Effect }, nextEffect: Effect): { Effect }\n\tassert(nextEffect.remaining > 0, "effects need positive duration")\n\tlocal next = {}\n\tlocal replaced = false\n\tfor _, effect in ipairs(effects) do\n\t\tif effect.id == nextEffect.id then\n\t\t\t table.insert(next, nextEffect)\n\t\t\t replaced = true\n\t\telse\n\t\t\t table.insert(next, effect)\n\t\tend\n\tend\n\tif not replaced then table.insert(next, nextEffect) end\n\treturn next\nend\n\nfunction StatusEffects.tick(health: number, effects: { Effect }, deltaTime: number): TickResult\n\tassert(deltaTime >= 0, "deltaTime cannot be negative")\n\tlocal damage = 0\n\tlocal speedMultiplier = 1\n\tlocal next = {}\n\tfor _, effect in ipairs(effects) do\n\t\tdamage += effect.damagePerSecond * deltaTime\n\t\tspeedMultiplier *= effect.speedMultiplier\n\t\tlocal remaining = effect.remaining - deltaTime\n\t\tif remaining > 0 then table.insert(next, { id = effect.id, remaining = remaining, damagePerSecond = effect.damagePerSecond, speedMultiplier = effect.speedMultiplier }) end\n\tend\n\treturn { health = math.max(0, health - damage), effects = next, speedMultiplier = speedMultiplier }\nend\n\nreturn StatusEffects\n` },
    { path: "src/shared/domain/Cooldowns.luau", content: `-- Generated pure cooldown mechanics for towers, abilities, and item actions.\n\nlocal Cooldowns = {}\n\nfunction Cooldowns.ready(remaining: number): boolean\n\treturn remaining <= 0\nend\n\nfunction Cooldowns.consume(duration: number): number\n\tassert(duration >= 0, "cooldown duration cannot be negative")\n\treturn duration\nend\n\nfunction Cooldowns.tick(remaining: number, deltaTime: number): number\n\tassert(deltaTime >= 0, "deltaTime cannot be negative")\n\treturn math.max(0, remaining - deltaTime)\nend\n\nreturn Cooldowns\n` },
